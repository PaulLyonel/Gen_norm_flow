...
def make_image_multiple(samples, img_name,directory='Images',ind=None):
    cmap = plt.cm.tab20
    plt.axis("off")
    no_pictures = len(samples)
    fig, axes = plt.subplots(figsize=[12,12], nrows=no_pictures, gridspec_kw={'wspace':0., 'hspace':0.});

    for j in range(no_pictures):
        axes[j].hist(samples[j][:,ind].cpu().data.numpy(),bins=100)
